...
async def _send_to_platform(platform, pconfig, chat_id, message, thread_id=None, media_files=None, force_document=False, args=None):
    ...
    if media_files and not message.strip():
        return {
            "error": (
                f"send_message MEDIA delivery is currently only supported for telegram, discord, matrix, weixin, signal, yuanbao, feishu, nextcloud_talk, whatsapp and slack; "
    ...
    if media_files:
        warning = (
            ...
            "native send_message media delivery is currently only supported for telegram, discord, matrix, weixin, signal, yuanbao, feishu, nextcloud_talk, whatsapp and slack"
        )
...
async def _send_nextcloud_talk(pconfig, chat_id, message, media_files=None, thread_id=None):
    ...
    media_files = media_files or []
    metadata = {"thread_id": thread_id} if thread_id else None
    # The live gateway session belongs to its event loop, so standalone sends
    # must use an ephemeral adapter on the tool loop.
