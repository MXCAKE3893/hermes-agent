...
def test_nextcloud_talk_checker_accepts_shared_secret():
    config = MagicMock()
    config.extra = {"secret": "shared-secret"}
    config.token = None

    assert _PLATFORM_CONNECTED_CHECKERS[Platform.NEXTCLOUD_TALK](config) is True
